...
{
	public partial class ZedGraphChart : Form, MarketSage.Library.Chart
	{
        Log log=null;
	    TimeStamp firstTime;
		StockPointList spl;
		List<PointPairList> lineList;
		//List<Model> indicators; 
		object chartLocker = new Object();
		bool showPriceGraph = true;
		static Interval initialInterval=null;
	    ...
	    public ZedGraphChart()
		{
		    ...
		    //indicators = new List<Model>();
		}
		
		void ChartLoad(object sender, EventArgs e)
		{
		...
		void ChartResize(object sender, EventArgs e)
		{
			SetSize();
		}
		...
		private void SetSize()
		{
		   ...
		   dataGraph.Size = new Size( ClientRectangle.Width - 25,
		                           ClientRectangle.Height - toolStripStatusXY.Height - logTextBox.Height - 20);
		   logTextBox.Location = new Point( 10, ClientRectangle.Height - logTextBox.Height - toolStripStatusXY.Height - 10);
		   logTextBox.Width = ClientRectangle.Width - 25;
		   checkBoxOnTop.Location = new Point(ClientRectangle.Width - 80,5);
		   audioNotify.Location = new Point(ClientRectangle.Width - 170,5);
		}
				...
				try {
					switch( clip) {
						case Audio.RisingVolume:
							fileName = @"c:\TickZoom\Media\risingVolume.wav";
						    SoundPlayer simpleSound = new SoundPlayer(fileName);
						    simpleSound.Play();
						    break;
						case Audio.IntervalChime:
						    fileName = @"c:\TickZoom\Media\intervalChime.wav";
						    simpleSound = new SoundPlayer(fileName);
						    simpleSound.Play();
						    break;
					}
				} catch( Exception e) {
					log.WriteLine("Error playing " + fileName + ": " + e);
				}
			}
		}
         */
		...
		GraphPane createPane() {
			...
			if( chartType == ChartType.Bar) {
			    ...
			    size.AddSeconds(interval.Seconds);
			    _clusterWidth = size;
			}
     ...
     */
		...
		//Dictionary<Model,PointPairList> repeatList = new Dictionary<Model,PointPairList>();
	    			...
	    			if( indicator.Count > 0) { 
						...
						switch( indicator.Drawing.GraphType ) {
		    				...
		    				case GraphType.Histogram:
		    					int startBar = 0; 
			    				ppair = lineList[j][updateSeries.CurrentBar-1];
			    				ppair.X = time;
			    				ppair.Y = val;
			    				ppair.Z = colorIndex;
			    				startBar ++;
	...
	//		    					}
             ...
             */
			    ...
			    if( showPriceGraph) {
		    		...
		    		if( spl.Count < updateSeries.CurrentBar ) {
					    ...
					    pt = (StockPt) spl.GetAt(updateSeries.CurrentBar-1);
				...
				if( spl.Count > 0 ) {
					...
					if( isCompactMode) {
						toolStripStatusXY.Text = time.ToLongTimeString() + ", " +
							...
							"O:" + startPair.Open.ToString("f0") + ", " + 
							"H:" + startPair.High.ToString("f0") + ", " + 
							"L:" + startPair.Low.ToString("f0") + ", " + 
							"C:" + startPair.Close.ToString("f0") + ", " +
							"Bar: " + (dragIndex+1) + ", " +
							"Period: " + interval;
					}
				}
		...
		public bool ProcessKeys(Keys keyData) {
			...
			if( keyData == Keys.Up ) {
				//strategy.Signal.GoLong();
				blnProcess = true;
			}
			if( keyData == Keys.Down ) {
				//strategy.Signal.GoShort();
				blnProcess = true;
			}
			if( keyData == Keys.Right) {
				//strategy.Signal.Reverse();
				blnProcess = true;
			}
			if( keyData == Keys.Insert || keyData == Keys.D0 || keyData == Keys.NumPad0 ) {
				//strategy.Signal.GoFlat();
				blnProcess = true;
			}
			return blnProcess;
		}
